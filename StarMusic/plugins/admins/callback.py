...
from StarMusic import YouTube, app
from StarMusic.core.call import Star
from StarMusic.misc import SUDOERS, db
from StarMusic.utils.database import (is_active_chat,
...
from StarMusic.utils.formatters import seconds_to_min
from StarMusic.utils.inline.play import (panel_markup_1,
                                          panel_markup_2,
                                          panel_markup_3,
                                          stream_markup,
                                          telegram_markup)
from StarMusic.utils.stream.autoclear import auto_clean
from StarMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Star.pause_stream(chat_id)
        ...
        await Star.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Star.mute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        await Star.unmute_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Star.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Star.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Star.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
