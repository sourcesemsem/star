...
from StarMusic import (Apple, Resso, SoundCloud, Spotify, Telegram,
                        YouTube, app)
from StarMusic.core.call import Star
from StarMusic.utils import seconds_to_min, time_to_seconds
from StarMusic.utils.channelplay import get_channeplayCB
from StarMusic.utils.database import is_video_allowed
from StarMusic.utils.decorators.language import languageCB
from StarMusic.utils.decorators.play import PlayWrapper
from StarMusic.utils.formatters import formats
from StarMusic.utils.inline.play import (livestream_markup,
                                          playlist_markup,
                                          slider_markup, track_markup)
from StarMusic.utils.inline.playlist import botplaylist_markup
from StarMusic.utils.logger import play_logs
from StarMusic.utils.stream.stream import stream
...
async def play_commnd(
    ...
    if audio_telegram:
        ...
        if await YouTube.exists(url):
            ...
            try:
                await Star.stream_call(url)
            except NoActiveGroupCall:
                await mystic.edit_text(
                    "هناك مشكلة في البوت. يرجى إبلاغ المالك الخاص بي واطلب منهم التحقق من مجموعة المسجل."
                )
                return await app.send_message(
                    config.LOG_GROUP_ID,
                    "الرجاء تشغيل الدردشة الصوتية .. لا يستطيع البوت دفق عناوين url..",
                )
            except Exception as e:
                return await mystic.edit_text(
...
@app.on_callback_query(
    filters.regex("StarPlaylists") & ~BANNED_USERS
)
@languageCB
